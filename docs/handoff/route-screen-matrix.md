...
**守衛**：`apps/admin/src/app/core/session/demo-session.guard.ts:9-12`。
...
目前只有**一種**守衛：`demoSessionGuard`。它掛在 17 筆 `loadComponent` 路由中的 15 筆上（`canActivate` 共出現 15 次），判斷邏輯只有一行：
...
`/` 與 `/login` 是**唯二無守衛**的路由。`/use/:assistantId` **有守衛**——這點對外部嵌入情境是個問題，見第 5 節。
...
| 連線失敗 | 管道狀態 `needs-attention` | 發布專屬，由 `?demoScenario=disconnected-channel` 觸發（`mock-demo-repository.ts:799`、`:2285`） |
| 登入逾時 | 守衛回傳 `UrlTree('/login')` | `demo-session.guard.ts:11`；`/login` 顯示逾時說明（`demo-login-page.component.ts:26-31`） |
...
| `/app/assistants/:id/:tab` `:44` | `features/assistants/assistant-detail/assistant-detail-page.component.ts` | `listAssistantConfigurations` `:74`；`getAssistantAnalytics` `:85`；`publishing` 分頁內嵌 `assistant-publishing.component.ts`（見下） | 空白（使用紀錄，`assistant-detail-page.component.html:46`）、成功、部分成功、權限不足（`:63`）、登入逾時 | `assistant-list.cy.ts`、`private-conversations.cy.ts`、`publishing.cy.ts`、`error-states.cy.ts`、`accessibility.cy.ts`、`responsive.cy.ts` |
...
| `/use/:assistantId` `:123` | `features/assistant-use/chat-shell/chat-shell-page.component.ts` → `chat-conversation.component.ts`，**單欄、無側欄** | `getAssistantChat` `:100`（**永遠不帶 `threadId`**）；`sendChatMessage` `:137`；`reviewChatForm` `:183`；`submitChatForm` `:205` | 空白、載入中（`chat-conversation.component.html:4`）、成功、部分成功、**無結果**、權限不足（`.html:8`）、登入逾時 | `private-conversations.cy.ts`、`consented-submission.cy.ts`、`chat-history.cy.ts`、`error-states.cy.ts`、`accessibility.cy.ts`、`responsive.cy.ts` |
| `/use/:assistantId?embed=1` | 同上，`header` 由 `full` 切成 `minimal`（`chat-shell-page.component.ts:17`、`:37`） | 同上 | 同上，但**沒有頁首、返回連結與品牌外框**；標題只留視覺隱藏版（`chat-conversation.component.ts:82`） | `chat-history.cy.ts` |

`/app/chat/:assistantId/:conversationId` 的第三段參數名稱是 **`conversationId`**（`app.routes.ts:115`），但 repository 契約叫它 `threadId`（`demo-repository.ts:442`）。同一個東西兩個名字，正式版應該統一。
...
`core/repositories/demo-scenario-param.ts:9-17` 讀網址參數，`core/repositories/tokens.ts:16-19` 在 DI factory 套用。合法值五種：`ready`、`loading`、`partial-failure`、`permission-denied`、`disconnected-channel`（`demo-repository.ts:66-72`）。
...
### 5.2 `/use/:assistantId` 掛著 `demoSessionGuard` 是個矛盾

這條路由的用途是**嵌入客戶官網的 iframe** 與**從 LINE 開啟**（元件註解 `chat-shell-page.component.ts:7-9`），但它目前和工作區路由一樣需要先選過 Demo 身分（`app.routes.ts:124`）。正式版必須決定：

- 未登入訪客怎麼使用嵌入的助理（匿名 session？一次性 token？）。
- 匿名訪客的對話屬於誰、能不能保存、能不能提交表單（表單需要同意紀錄，同意紀錄需要主體）。
- `?embed=1` 只是視覺開關（`chat-shell-page.component.ts:37`），**沒有任何安全意義**——它不做 origin 檢查、不驗證 referrer、不限制 iframe 來源。真正的來源限制必須由後端的允許網域與 CSP `frame-ancestors` 執行。

設計文件對訪客 session 隔離已有要求，見 `docs/plans/2026-09-18-sme-ai-assistant-ux-demo-design.md:194-199`。

### 5.3 所有 `:id` 參數都未經驗證就丟給 repository

`/app/knowledge/:id/:tab`、`/app/databases/:id/:tab`、`/app/assistants/:id/:tab`、`/use/:assistantId`、`/app/chat/:assistantId/:conversationId` 的 id 全部直接來自網址。契約上這些方法的 id 參數型別是 `string` 而非字面值 union（例 `demo-repository.ts:342`、`:384`、`:441`），**這是刻意的**——因為「不存在」與「無權限」要回同一個結果，前端不能先在本地判斷 id 是否合法。正式版必須維持這個性質。
