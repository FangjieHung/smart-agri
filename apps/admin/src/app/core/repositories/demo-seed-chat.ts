...
/**
 * 撤回說明：已登入帳號的版本。
 * 明確說出撤回之後接收單位還看得到什麼——只有時間軌跡，沒有內容——不承諾整筆消失。
 */
export const CHAT_WITHDRAWAL_NOTICE =
  '送出後可以隨時在這段對話的送出收據上撤回。撤回後接收單位的收集紀錄會移除這筆資料的內容，也不再計入趨勢比較，只保留一筆「曾提交、已撤回」的軌跡（提交與撤回時間、來源）。';

/**
 * 撤回說明：未登入官網訪客的版本。
 * 訪客只有一個存在分頁裡的隨機 id，關掉分頁就再也指認不到這筆紀錄，所以直說。
 */
export const CHAT_VISITOR_WITHDRAWAL_NOTICE =
  '送出後可以在這段對話的送出收據上撤回，接收單位會移除內容、只留下「曾提交、已撤回」的軌跡。但你沒有登入：這筆資料只認得這個瀏覽器分頁，關閉分頁後就沒有任何方式能證明它是你的，也就無法再撤回。';

/** 已撤回之後收據上的說明。 */
export const CHAT_WITHDRAWN_NOTICE =
  '你已撤回這筆資料。接收單位的收集紀錄已移除內容，也不再計入趨勢比較，只留下一筆「曾提交、已撤回」的軌跡。';

/** 舊版收據沒有紀錄 id，指認不到紀錄時照實說，不提供假的撤回入口。 */
export const CHAT_WITHDRAWAL_UNAVAILABLE_NOTICE =
  '這張收據指認不到對應的紀錄（是舊版本的收據，或紀錄已不存在），所以無法從這裡撤回。請直接聯絡接收單位。';
